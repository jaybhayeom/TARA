...
export const THEMES: ThemeConfig[] = [
  { id:"midnight", name:"Midnight", emoji:"🌙", bg:"#0d0d15", surface:"rgba(18,18,28,0.97)",  blobColor:"rgba(139,92,246,0.38)",  accentColor:"#a78bfa" },
...
export function Onboarding({ onComplete }: Props) {
  ...
  function finish() {
    setExiting(true);
    setTimeout(() => onComplete({ name: name.trim() || "You", role, agent, themeId }), 820);
  }
  ...
  return (
    <div style={{
      ...
      {/* ── Skip ── */}
      {phase < 6 && !exiting && (
        <button onClick={() => { clear(); setFadeOut(false); setPhase(6); }}
      ...
      <div style={{
        ...
        {phase === 6 && (
          <div style={{ width:"min(460px,calc(100vw-48px))", animation:"ob-fade-up 0.55s ease-out both" }}>
            ...
            <PrimaryBtn label="Let's go" disabled={false} onClick={finish} accent={theme.accentColor} />
          </div>
        )}
